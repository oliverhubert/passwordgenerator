...
String generatePassword(String name, String code) {
  ...
  bool l,u,d,o;
    ...
    for (int i = 0; i < 6; i++ ) {
			...
			if (newpwd[i].islower())
				l = true;
			else if (newpwd[i].isupper())
				u = true;
			else if (newpwd[i].isdigit())
				d = true;
			else
				o = true;
    }
...
String validChar = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyz-_<>+*?=()/&%$!,.;:";
...
//	return(validChar[myRand()%len(validChar)]);
  return(validChar[myRand() % validChar.length]);
}
